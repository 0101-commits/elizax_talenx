...
### 20차: 신호 카탈로그를 elizax 안에서 본다 · 엑셀에 「부르는 질문」 열

**질문 열** — W5 제출본 엑셀(`성과평가 AI Agent_AX Consulting_W5_signal catalogue_260803.xlsx`)에 「알림(Signal) 문구(예시)」 바로 뒤 **「이 알림을 부르는 질문 (모두 예시)」** 열을 신설했다. 값은 새로 쓴 문장이 아니라 이 리포 `js/ez_signal_chat.js` 의 `QMAP` 150건을 그대로 옮긴 것이다 — **제품이 단일 원천**이라 표에 적힌 문장을 그대로 입력하면 그 알림이 열린다. 「읽는 법」(Outlook) 시트 「본표 열 뜻」에도 새 열 설명 한 줄을 넣었다.

그 약속을 기계로 센다 — `node scripts/check_signal_questions.js` : 질문 150건의 **빠짐 0 · 중복 0 · 라우팅 어긋남 0**(질문을 그대로 넣으면 `matchAny` 가 그 신호를 고르는지) · 길이 18~36자. QMAP·낱말 사전(`KEYS`·`HINT`)을 고칠 때마다 다시 돌린다.

**카탈로그 열람** — 예전 「이런 것도 물어볼 수 있어요」 질문 목록을 **신호 카탈로그 열람**으로 넓혔다(`tx_elizax.js` `openCatalog`).

| 무엇 | 어떻게 |
| --- | --- |
| 진입 | 빈 대화창 아래 「물어볼 수 있는 것 전부 보기」 · **알림 탭 「오게 되어 있는 알림 150건을 미리 볼 수 있어요」** |
| 목록 | 한 줄에 두 층 — 부르는 질문 + 그 알림이 화면에 내보내는 문장. 오른쪽에 「지금 확인 가능 / 기록 준비 중」 |
| 고르기 | 검색(질문·문장·받는 사람) + 칩 — 단계 4종 · 지금 확인 가능 · 내가 받는 것. 머리에 「N건 보임 · 지금 확인 가능 15건 · 전체 150건」 |
| 펼치면 | 설계 원본 그대로 — 이 알림이 오는 때 · 화면에 나가는 문장 · 근거 줄 전체 · 기준값(조정 범위) · 비교 대상 · 판단할 때 읽는 기록 · 할 수 있는 일(남는 기록까지) · 알림 조절 · 켜기 전에 남은 일 |
| 보내기 | 「이 질문 보내기」 → 그 주제를 걸고 대화로 (`EZSignalChat.ask`) |

열람 화면은 **답변 화면이 아니라 설계 원본을 보는 자리**라, 답변에서는 감추는 분류 이름(단계·받는 사람·난이도)을 오히려 밝혀 적는다. 기계 식별자는 여기서도 내보내지 않는다 — 근거·참조는 `scrub()` 을 지나 사람이 읽는 이름으로 바뀌고, 기준값의 내부 코드(`TH-…`)는 그리지 않는다.

### 20-3차: 신호 답변은 카탈로그가 만든다 (모델이 아니다)

**문제** — 신호 근거는 `[참고 자료 — 사용자에게 그대로 보여 주지 마세요]` 블록으로 payload 에만 실렸다. 모델이 자기 문장으로 녹이면 **카탈로그의 알림 문구가 화면에서 사라졌다.** "질문을 넣었는데 그 알림이 안 보인다"가 그것이다.

**고침** — 질문이 신호에 걸리면 모델을 거치지 않고 답의 뼈대를 화면에 먼저 그린다(`EZSignalChat.answerBlocks` → `tx_elizax.js buildSigNode`, 메시지 role `"sig"`). 순서가 뜻이다.

| 층 | 내용 |
| --- | --- |
| ① 알림 문구 | 그 신호가 화면에 내보내는 한 문장 (실측 치환) |
| ② 기준 시점 | as-of 한 줄 |
| ③ 근거 | 사실·비교·추이·연결 줄. 추정은 「(추정)」 |
| ④ 기준값 | 「회사가 보는 잠정 기준 N · 지금 측정값 M」 |
| ⑤ 할 수 있는 일 | 카탈로그 처리 상위 2개 + 「이 알림 자세히」(카탈로그 그 행을 펼쳐 연다) |

**세 갈래를 가른다** — 섞으면 없는 문제를 있다고 말한다.

- 실계산 + 참 → 알림 문구 그대로
- 실계산 + 거짓 → 「지금은 이 알림이 뜰 상태가 아니에요」 + 언제 뜨는 알림인지 + 실측 근거
- 실계산 못 함(135건) → 「숫자는 예시예요」 밝히고 카탈로그 예시 근거 2줄 + 먼저 남아야 하는 기록

모델의 몫은 그 뒤 한두 문장뿐이다 — payload 에 「이미 화면에 보여 준 것은 되풀이하지 말고 이어서 무엇부터 하면 좋은지만」 지시를 넣었고, 신호 답변이 있으면 「확인 중」 연출 카드도 넣지 않는다(답이 먼저 떠 있는데 뒤에서 찾는 척하면 순서가 거짓말이 된다).

### 20-4차: 예시가 실측으로 — 원천 데이터 주입 + 점화 57건

**문제** — 카탈로그 150건 중 「AI 판단 가능」이 「데이터 보완 후」 47건·「이력 축적 후」 40건이었다. 판단할 원천이 `TALENX_DATA` 에 아예 없어 예시값으로만 보여 줄 수 있었다.

**원천 데이터** `scripts/enrich_signal_data.py`(멱등·합성 표기·정합 전수 검산). 카탈로그의 「새로 만들 기록」 132종을 뜻으로 묶은 15개 컬렉션.

| 컬렉션 | 무엇을 판정하게 되었나 |
| --- | --- |
| `periods` 9건 | 기한 임박·지남 — 기준 시점 2026-07-16 대비 하반기 목표 확정 D+3, 체크인 D+1, 다면진단 지남 |
| `objectives`·`keyResults` 시각 | 저장 뒤 경과일 · 미확정 목표 · 손대지 않은 기간 |
| `krProgress` 1,898건 | 진척 추이 — 정체 구간·급등 구간을 심어 두었다 |
| `evaluatorMap`·`evalStatus` 각 221건 | 평가자 매핑 공백 · 제출 지연 · 확정 지연 |
| `selfEval`·`gradeHistory` | 자기평가 부실·숫자 근거 없음 · 등급 변경 사유 없음 |
| `policy` | 등급 분포 권고·경계·난이도 분포·요소 가중치·기준일 — 제도 대비 이탈 판정 |
| `notifyLog`·`requestLog` | 보낸 알림을 안 열었다 · 요청 회신 없다 |
| `meetingStore` 126건 | 면담 안 함 · 합의 항목 미이행 |
| `feedbackLog`·`msfAssign` 120건·`upwardResp` | 초안만 두고 안 보냄 · 다면진단·상향 미제출 |
| `devPlan` 40건 | 낮은 등급인데 육성 계획 미등록 |
| `checkins` 360 → 3,357건 | 26주 주간 회차, 다섯 중 하나는 비우고 반복 걸림돌 3종을 특정 조직에 집중 |

**점화 판정 재정의** — 카탈로그 `now`(설계 시점 판단) 대신 **엔진에 판정 함수가 있는가**로 가른다. `EZSignalEngine.registerEval(id, fn)`·`hasEval(id)`·`helpers` 를 열어 판정 함수를 별도 파일에서 등록한다(`js/ez_signal_eval2~5.js` — 목표수립·중간점검·평가·피드백). 화면·대화·처리 배선의 「지금 확인 가능」 판정도 전부 이 기준으로 옮겼다.

**결과 15건 → 57건.** 예전에 「예시로 보여드려요」였던 답이 실측으로 바뀐다 — 예를 들어 「목표 확정일 전에 내 목표가 비어 있는지 봐줘」는 이제 *"목표 확정 마감일 2026년 7월 19일까지 3일 남았어요 · 제도 기준 3건인데 지금 저장분은 1건이에요"* 로 답하고, 조건(0건)에 못 미치므로 **「지금은 이 알림이 뜰 상태가 아니에요」라고 정직하게 말한다.**

`node scripts/check_signal_thresholds.js` — 기준값 120개가 카탈로그 값과 일치하는지, 카탈로그를 고치면 판정선이 따라오는지 센다.

### 처리 단추를 누른 다음 (20-4차)

| 단계 | 화면 |
| --- | --- |
| 누른 직후 | 「[확인 단추]를 누르면 반영돼요 → 남는 기록 : …」 — 아직 안 끝났으므로 「했다」고 말하지 않는다 |
| 실제로 끝나면 | 카탈로그 「처리 후 안내」(`done.title`·`desc`) 그대로 |
| 두 단계 모두 | 아래에 **이어 물을 말 3개**. 누르면 그 대화로 넘어간다 |
